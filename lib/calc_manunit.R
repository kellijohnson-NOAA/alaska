...
#' Must use backslahes, such that the call to \code{system} will work.
#' For example: \code{"c:\\SaTScan\\SatScanBatch.exe"}, if on Windows.
...
#' # temp <- read_results(report = Report)
...
calc_manunit <- function(readin, dir = getwd(), file = NULL,
  ...
  smallbuffer <- sqrt(rgeos::gArea(bounds$poly)) * 0.0001
  info@data$true <- sp::over(info,
    rgeos::gBuffer(pol.true, width = smallbuffer, byid = TRUE))
  ...
  raster::projection(pol.choose) <- raster::projection(info, asText = FALSE)
  if (file.exists(paste0(shapefile, ".shp"))) {
    pol.hull <- maptools::readShapePoly(shapefile)
    pol.hull <- sp::SpatialPolygons(pol.hull@polygons)
    raster::projection(pol.hull) <-
      raster::projection("+proj=longlat +ellps=WGS84")
